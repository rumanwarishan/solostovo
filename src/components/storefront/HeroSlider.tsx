...
export function HeroSlider({
  ...
  return (
    ...
    >
      <div className="relative w-full sm:aspect-[21/9]">
        {/* Media: a compact, self-contained box on mobile; fills the parent
            from sm up, where the text overlays it directly. */}
        <div className="relative aspect-[16/9] w-full overflow-hidden sm:absolute sm:inset-0 sm:aspect-auto">
          {slides.map((s, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/15 sm:bg-black/30" />
            </div>
          ))}
        ...
        {/* Text: flows in normal document order below the video on mobile;
            overlays the media directly from sm up. */}
        <div className="container-page relative py-6 sm:absolute sm:inset-0 sm:flex sm:items-center sm:py-0">
          <div className="max-w-xl text-brand-ink sm:text-white">
            {slide.eyebrow && (
              <span className="text-xs font-semibold uppercase tracking-widest text-brand-ink/60 sm:text-white/80">
                {slide.eyebrow}
              </span>
            )}
            ...
            {slide.subtitle && (
              <p className="mt-4 max-w-md text-brand-ink/70 sm:text-white/80">{slide.subtitle}</p>
            )}
            <div className="mt-7 flex flex-wrap gap-3">
              {slide.ctaLabel && (
                <Link
                  href={slide.ctaHref || "#"}
                  className="inline-flex items-center gap-2 rounded-full bg-brand-ink px-6 py-3 text-sm font-medium text-white hover:bg-brand-ink/90 sm:bg-white sm:text-brand-ink sm:hover:bg-white/90"
                >
                  {slide.ctaLabel}
              ...
              {slide.secondaryCtaLabel && (
                <Link
                  ...
                  className="inline-flex items-center rounded-full border border-brand-ink px-6 py-3 text-sm font-medium text-brand-ink hover:bg-brand-ink/5 sm:border-white sm:text-white sm:hover:bg-white/10"
                >
                  {slide.secondaryCtaLabel}
                </Link>
              )}
